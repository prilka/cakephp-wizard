...
class WizardHelper extends Helper {
    ...
    public function configure( $key = null ) {
        if( $key == null ) {
            return $this->request->session()->read( $this->sessionRootKey . '.config' );
        } else {
            $wizardData = $this->request->session()->read( $this->sessionRootKey . '.config.' . $key );
            if( !empty( $wizardData ) ) {
                return $wizardData;
            } else {
                return null;
     ...
     */
    public function link( $title, $step = null, $htmlAttributes = [], $confirmMessage = false ) {
        if( $step == null ) {
            $step = $title;
        }
        $url = [
            'action' => $this->configure( 'action' ),
            $step,
        ];
    ...
    public function stepNumber( $step = null, $shiftIndex = 1 ) {
        if( $step == null ) {
            $step = $this->configure( 'activeStep' );
        }
        $steps = $this->configure( 'steps' );
     ...
     * @return string
     */
    public function progressMenu( $titles = [], $attributes = [], $htmlAttributes = [], $confirmMessage = false ) {
        $wizardConfig = $this->configure();
        extract( $wizardConfig );
        $wizardAction = $this->configure( 'action' );
    ...
    private function __getStepUrl( $step ) {
        $wizardAction = $this->configure( 'action' );
        if( $this->configure( 'persistUrlParams' ) ) {
